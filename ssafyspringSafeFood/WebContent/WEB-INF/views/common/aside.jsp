...
<c:if test="${empty sessionScope.id}">
 ...
 <li class="last"><p class="trigger-custom"
		...
		<div id="modal-custom" data-iziModal-group="grupo1">
			...
			<section class="hide">
<form action="<%=request.getContextPath()%>/login.do" method="post" id='_fo3'>


					<input type="text" name='id' placeholder="Username" id='lid'> 
					<input type="password"  name='password' placeholder="Password" id='lpwd'>
						</form>
				<footer>
			...
			<section>
			<form action="<%=request.getContextPath()%>/signin.do" method="post" id='_fo4'>
  ...
  <c:if test="${not empty sessionScope.id}">
   <li><p>[${id}]님 반갑습니다.<br/><%=sdf.format(new Date()) %>
		<a href="#" data-izimodal-open="#modal-custom">회원 정보 </a> <a href='./logout.do'>로그아웃 </a>
		<div id="modal-custom" data-iziModal-group="grupo1">
			<button data-iziModal-close class="icon-close">x</button>
			<header>
			<p>Member Info</p>
			...
			<section>
			<form action="<%=request.getContextPath()%>/change.do" method="post" id='_fo5'>
				...
				<footer>
					...
					<a id='' href='<%=request.getContextPath()%>/delete.do'>Delete Member</a>
